...
class StudentProgress(dict):
    ...
    def read_answers(self):
        ...
        log.info(f"Read {len(self)} answers")
    ...
    def save_answers(self):
        ...
        with open(self.ANSWER_FILE_PATH, "w") as f:
            f.write(
                self.PROBLEM_SPLIT.join(
                    v.to_string() for k, v in sorted(self.items())
                )
            )

        log.info(f"Saved {len(self)} answers")
...
class Database:
    ...
    def result_to_str(description, results):
            ...
            cols.append(
                {
                    "name": col.name,
                    "len": max(
                        len(col.name), COL_TYPE_TO_LEN.get(col.type_code, 10)
                    ),
                }
            )
        ...
        if len(results) <= MAX_ROWS:
            out += f"({len(results)} rows)"
        else:
            out += (
                f"({len(results)} rows, truncated for display at {MAX_ROWS})"
            )
    ...
    def get_raw_result(self, attempt, error_on_empty=False):
        ...
        try:
            ...
            err = f"There was a problem with your SQL syntax:\n\n\t{e}\n"
            if error_on_empty:
...
class SQLQuiz:
    ...
    def show_problem(self):
        ...
        print(f"\nProblem {self.current_problem.num}")
        print("----------\n")
    ...
    def get_solution(self):
        ...
        while True:
            try:
                if not sql:
                    print()
                    line = input(f"SQL [{problem.num}]> ")
                else:
                    line = input(f"... [{problem.num}]> ")
            except EOFError:
                return False
